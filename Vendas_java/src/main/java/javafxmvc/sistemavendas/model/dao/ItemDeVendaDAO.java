package javafxmvc.sistemavendas.model.dao;
...
public class ItemDeVendaDAO {
    ...
    public boolean inserir(ItemDeVenda itemDeVenda) {
        ...
        try {
            Venda venda = itemDeVenda.getVenda();
            if (venda == null || venda.getCdVenda() == 0) {
                throw new IllegalArgumentException("Invalid Venda associated with ItemDeVenda");
            }

            VendaDAO vendaDAO = new VendaDAO();
            vendaDAO.setConnection(connection);
            Venda existingVenda = vendaDAO.buscar(venda);
            if (existingVenda == null) {
                throw new IllegalArgumentException("Venda does not exist in the database");
            }
